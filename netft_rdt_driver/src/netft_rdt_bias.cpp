...
NetFTRDTDriverBias::NetFTRDTDriverBias(ros::NodeHandle &nh, double rot,
                                       const tf::Vector3 &scale_F, double alpha,
                                       std::size_t num_points)
    : num_points(num_points), scale_F(scale_F), alpha(alpha) {
  count = 0;
  force_b.x = 0;
  force_b.y = 0;
  force_b.z = 0;
  ...
  service_server = nh.advertiseService(
      "bias_cmd", &NetFTRDTDriverBias::service_callback, this);
...
bool NetFTRDTDriverBias::service_callback(
    netft_rdt_msgs::String_cmd::Request &request,
    netft_rdt_msgs::String_cmd::Response &response) {
